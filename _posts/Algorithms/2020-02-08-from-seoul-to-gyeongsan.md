...
## 풀이
단순하게 푸는 방법은 모든 경우의 수를 고려하는 것이다. 이 경우 $$O(2^N)$$의 시간 복잡도로 This formula $$f(x) = x^2$$ is an example.
처음에는 상식적으로 도보로 이동하는 것보다 자전거로 이동하는 것이 모든 구간에서 더 빠를 것이라 생각했다. 또, 도보로